...
/** not-@type {!BigUint64Array} */ HEAPU64;
      ...
      stream_ops: {
        ...
        write: (stream, buffer, offset, length) => {
          ...
          if (buffer === HEAP8) {
            ...
            throw new Error(
              `Unexpected buffer type: ${buffer.constructor.name}`
            );
          }
    ...
    function ___syscall_fallocate(fd, mode, offset, len) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(10, 0, 1, fd, mode, offset, len);
    ...
    function ___syscall_fstatfs64(fd, size, buf) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(15, 0, 1, fd, size, buf);
      size >>>= 0;
      buf >>>= 0;
      try {
        var stream = SYSCALLS.getStreamFromFD(fd);
    ...
    function ___syscall_listen(fd, backlog) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(23, 0, 1, fd, backlog);
      try {
        var sock = getSocketFromFD(fd);
    ...
    function ___syscall_mkdirat(dirfd, path, mode) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(25, 0, 1, dirfd, path, mode);
      path >>>= 0;
      try {
        path = SYSCALLS.getStr(path);
        path = SYSCALLS.calculateAt(dirfd, path);
    ...
    function ___syscall_newfstatat(dirfd, path, buf, flags) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(26, 0, 1, dirfd, path, buf, flags);
      path >>>= 0;
      buf >>>= 0;
      try {
        path = SYSCALLS.getStr(path);
    ...
    function ___syscall_openat(dirfd, path, flags, varargs) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(27, 0, 1, dirfd, path, flags, varargs);
      path >>>= 0;
      varargs >>>= 0;
      SYSCALLS.varargs = varargs;
      try {
    ...
    function xterm_pty_old_poll(fds, nfds, timeout) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(29, 0, 1, fds, nfds, timeout);
      fds >>>= 0;
      try {
        var nonzero = 0;
    ...
    var PTY_waitForReadableWithAtomicImpl = function (atomicIndex) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(30, 0, 0, atomicIndex);
    ...
    var PTY_wrapPoll = impl =>
      PTY_handleSleep(wakeUp => {
        let result = impl();
    ...
    var ___syscall_poll = (fds, nfds, timeout) =>
      PTY_wrapPoll(() => xterm_pty_old_poll(fds, nfds, timeout));
    ...
    function _environ_get(__environ, environ_buf) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(47, 0, 1, __environ, environ_buf);
      __environ >>>= 0;
      environ_buf >>>= 0;
      var bufSize = 0;
      var envp = 0;
    ...
    function _environ_sizes_get(penviron_count, penviron_buf_size) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(48, 0, 1, penviron_count, penviron_buf_size);
      penviron_count >>>= 0;
      penviron_buf_size >>>= 0;
    ...
    function _fd_pread(fd, iov, iovcnt, offset, pnum) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(51, 0, 1, fd, iov, iovcnt, offset, pnum);
      iov >>>= 0;
      iovcnt >>>= 0;
      offset = bigintToI53Checked(offset);
      pnum >>>= 0;
    ...
    function _fd_pwrite(fd, iov, iovcnt, offset, pnum) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(52, 0, 1, fd, iov, iovcnt, offset, pnum);
      iov >>>= 0;
      iovcnt >>>= 0;
      offset = bigintToI53Checked(offset);
      pnum >>>= 0;
        ...
        if (result === 1006) {
          ...
          PTY_waitForReadable(type => {
            switch (type) {
              case 0:
                /* ready */ wakeUp(
                  xterm_pty_old_fd_read(fd, iov, iovcnt, pnum)
                );
                break;
    ...
    function _fd_seek(fd, offset, whence, newOffset) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(54, 0, 1, fd, offset, whence, newOffset);
      offset = bigintToI53Checked(offset);
      newOffset >>>= 0;
      try {
        if (isNaN(offset)) return 61;
    ...
    function _fd_write(fd, iov, iovcnt, pnum) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(56, 0, 1, fd, iov, iovcnt, pnum);
      iov >>>= 0;
      iovcnt >>>= 0;
      pnum >>>= 0;
      try {
    ...
    function _getaddrinfo(node, service, hint, out) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(57, 0, 1, node, service, hint, out);
      node >>>= 0;
      service >>>= 0;
      hint >>>= 0;
      out >>>= 0;
    ...
    function instantiate_wasm() {
      ...
      const counter_vec_size = memory_v.getInt32(
        export_vec_begin + export_vec_size,
        true
      );
      const counter_vec_begin = export_vec_begin + export_vec_size + 4;
      const tmp_body_size = memory_v.getInt32(
        counter_vec_begin + counter_vec_size,
        true
      );
      ...
      const wasmBytes = new Uint8Array(
        HEAP8.slice(wasm_begin, wasm_begin + wasm_size)
      );
      var helper = {};
    ...
    function remove_module_js() {
      const memory_v = new DataView(HEAP8.buffer);
      const remove_n = memory_v.getInt32(
        Module.__wasm32_tb.to_remove_instance_idx_ptr,
        true
      );
    ...
    function ffi_call_js(cif, fn, rvalue, avalue) {
      ...
      for (var i = 0; i < nfixedargs; i++) {
        ...
        switch (arg_type_id) {
          ...
          case 13:
            ...
            HEAP8.subarray(
              cur_stack_ptr >>> 0,
              (cur_stack_ptr + size) >>> 0
            ).set(HEAP8.subarray(arg_ptr >>> 0, (arg_ptr + size) >>> 0));
      ...
      if (nfixedargs != nargs) {
        ...
        for (var i = nargs - 1; i >= nfixedargs; i--) {
          ...
          switch (arg_type_id) {
            ...
            case 8:
              ...
              HEAPU16[((cur_stack_ptr >> 1) + 0) >>> 0] =
                HEAPU16[((arg_ptr >> 1) + 0) >>> 0];
              break;
            ...
            case 2:
              ...
              HEAPU32[((cur_stack_ptr >> 2) + 0) >>> 0] =
                HEAPU32[((arg_ptr >> 2) + 0) >>> 0];
              break;
            ...
            case 12:
              ...
              HEAPU32[((cur_stack_ptr >> 2) + 0) >>> 0] =
                HEAPU32[((arg_ptr >> 2) + 0) >>> 0];
              HEAPU32[((cur_stack_ptr >> 2) + 1) >>> 0] =
                HEAPU32[((arg_ptr >> 2) + 1) >>> 0];
              break;
            ...
            case 4:
              ...
              HEAPU32[((cur_stack_ptr >> 2) + 1) >>> 0] =
                HEAPU32[((arg_ptr >> 2) + 1) >>> 0];
              HEAPU32[((cur_stack_ptr >> 2) + 2) >>> 0] =
                HEAPU32[((arg_ptr >> 2) + 2) >>> 0];
              HEAPU32[((cur_stack_ptr >> 2) + 3) >>> 0] =
                HEAPU32[((arg_ptr >> 2) + 3) >>> 0];
              break;
            ...
            case 13:
              ...
              struct_arg_info.push([
                cur_stack_ptr,
                arg_ptr,
                HEAPU32[((arg_type_ptr >> 2) + 0) >>> 0],
                HEAPU16[(((arg_type_ptr + 4) >> 1) + 0) >>> 0]
              ]);
              break;
        ...
        for (var i = 0; i < struct_arg_info.length; i++) {
          ...
          ((cur_stack_ptr -= size), (cur_stack_ptr &= ~(align - 1)));
          HEAP8.subarray(cur_stack_ptr >>> 0, (cur_stack_ptr + size) >>> 0).set(
            HEAP8.subarray(arg_ptr >>> 0, (arg_ptr + size) >>> 0)
          );
      ...
      function trampoline() {
        ...
        for (; carg_idx < nfixedargs; carg_idx++) {
          ...
          switch (arg_type_id) {
            ...
            case 13:
              ...
              HEAP8.subarray(cur_ptr >>> 0, (cur_ptr + arg_size) >>> 0).set(
                HEAP8.subarray(cur_arg >>> 0, (cur_arg + arg_size) >>> 0)
              );
              HEAPU32[((args_ptr >> 2) + carg_idx) >>> 0] = cur_ptr;
              break;
        ...
        for (; carg_idx < nargs; carg_idx++) {
          ...
          if (arg_type_id === 13) {
            ...
            ((cur_ptr -= arg_size), (cur_ptr &= ~(arg_align - 1)));
            HEAP8.subarray(cur_ptr >>> 0, (cur_ptr + arg_size) >>> 0).set(
              HEAP8.subarray(struct_ptr >>> 0, (struct_ptr + arg_size) >>> 0)
            );
            HEAPU32[((args_ptr >> 2) + carg_idx) >>> 0] = cur_ptr;
          } else {
        ...
        stackAlloc(0);
        0;
        getWasmTableEntry(HEAPU32[((closure >> 2) + 2) >>> 0])(
          HEAPU32[((closure >> 2) + 1) >>> 0],
          ret_ptr,
          args_ptr,
          HEAPU32[((closure >> 2) + 3) >>> 0]
        );
        stackRestore(orig_stack_ptr);
        if (!ret_by_arg) {
          switch (sig[0]) {
    ...
    function assignWasmExports(wasmExports) {
      ...
      Module['__emscripten_proxy_main'] = __emscripten_proxy_main =
        wasmExports['_emscripten_proxy_main'];
      ...
      dynCalls['viiiijjii'] = dynCall_viiiijjii =
        wasmExports['dynCall_viiiijjii'];
      ...
      dynCalls['viiiiiiii'] = dynCall_viiiiiiii =
      ...
      dynCalls['iiiiiiiii'] = dynCall_iiiiiiiii =
      ...
      dynCalls['iiijijjii'] = dynCall_iiijijjii =
        wasmExports['dynCall_iiijijjii'];
      dynCalls['iiijiiiii'] = dynCall_iiijiiiii =
        wasmExports['dynCall_iiijiiiii'];
